...
class Post_htag extends AsyncTask<Void, Void, String> {

    private String url1;
    private String result1; // 요청 결과를 저장할 변수
    private ArrayList<Post> post;
    private ContentValues content;
    public Post_htag(String str) {
        this.content = new ContentValues();
        content.put("userId",MainActivity.user.getId());
        content.put("htag",str);
        this.url1 = "http://" + MainActivity.IP + "/post_htag.php";  //select_follower.php
    }

    public ArrayList<Post> search_htag(){
        try {
    ...
    public void doJSONParser(String string) {
        try {
            if(string == null) {
                ...
                JSONArray jsonArray = jsonObject.getJSONArray("htag_feed");
